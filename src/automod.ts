...
const warn = async (client: Client, channel: string, channelId: string, state: ChatUserstate, reason: string, publicText: string, deleteMessage: boolean = true) => {
...
export const executeAutomod = async (message: string, state: ChatUserstate, channel: string, client: Client) => {
  ...
  const settings = await getSettings(channelId) ?? { antiDuplicate: true, antiUpperCase: true };
  if (
    settings.antiUpperCase &&
    message.length > 10 &&
    countUpperCase(message) / (message.match(/[A-z]/g) ?? []).length > 0.8
  ) {
    await warn(client, channel, channelId, state, "Too many uppercase", "don't use that many uppercase");
  }

  const regexp = /(\S+)([\t ]*)(?:\1\2?){7,}/g
  if (settings.antiDuplicate && regexp.test(message) && message.length > 7) {
    await warn(client, channel, channelId, state, "Mass duplicated characters", "don't use that many duplicated characters");
  }
}